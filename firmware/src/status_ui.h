...
// Host → device status snapshot (little-endian). Padded inside TERM_PAYLOAD.
...
// systemd unit ActiveState (wire codes).
...
// Secondary one-line stats (0xFF = slot empty / section omitted if both empty).
